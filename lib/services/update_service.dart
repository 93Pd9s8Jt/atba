...
class UpdateService {
  ...
  Stream<Map<String, dynamic>> monitorItem<T extends DownloadableItem>(
    int itemId,
  ) async* {
    while (true) {
      try {
        ...
        final response = await (() async {
          ...
          switch (T) {
            case const (Torrent):
              return _apiService.getTorrentsList(
                torrentId: itemId,
                bypassCache: true,
              );
            case const (WebDownload):
              return _apiService.getWebDownloadsList(
                webId: itemId,
                bypassCache: true,
              );
            case const (Usenet):
              return _apiService.getUsenetDownloadsList(
                usenetId: itemId,
                bypassCache: true,
              );
            default:
